...
if(!$_POST){
	...
	foreach($exec as $row)
	{
		...
		echo '<tr>
				<td> <input id="next" type="radio" name="eid" value="'.$row["eid"].'">
						</td>
				<td>'.$row["aid"].'<input type="hidden" name="aid" value="'.$row["aid"].'">
						</td>
				<td>'.$row["lname"].'<input type="hidden" name="date" value="'.$_POST['paydate'].'">
						</td>
				<td>'.$row["name"].'
						</td>
				<td>'.$row["grade"].'
